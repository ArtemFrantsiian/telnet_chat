...
fn handle_broadcast(writer: &mut BufWriter<TcpStream>, ntf: BroadcastNotify) -> AppResult<bool> {
    match ntf {
        ...
        BroadcastNotify::Register { name, .. } => {
            try!(writeln!(writer, "User {} protect his name", name));
            try!(writer.flush());
        }
...
fn handle_unicast(writer: &mut BufWriter<TcpStream>, ntf: UnicastNotify) -> AppResult<bool> {
    match ntf {
        ...
        UnicastNotify::List(list) => {
            try!(writeln!(writer, "Members online:"));
            for mem in list {
        ...
        UnicastNotify::Register(succeed) => {
            if !succeed {
                try!(writeln!(writer, "=> register failed"));
                try!(writer.flush());
            }
        }
...
fn reader_loop(stream: TcpStream, req_tx: Sender<Request>) -> AppResult<()> {
    ...
    loop {
        ...
        let req_body = if !input.starts_with("/") {
            ...
            let mut it = input.splitn(3, is_whitespace);

            let command = it.next();
            let arg_f = it.next().map(|s| s.trim());
            let arg_s = it.next().map(|s| s.trim());  
            match (command, arg_f, arg_s) {
                (Some("/exit"), ..) => break,
                (Some("/rename"), Some(name), None) => {
                    let name = name.trim().to_string();
                    RequestBody::Rename { name, password: "".to_string() }
                }
                (Some("/rename"), Some(name), Some(password)) => {
                    let name = name.trim().to_string();
                    let password = password.trim().to_string();
                    RequestBody::Rename { name, password }
                }
                (Some("/register"), Some(name), Some(password)) => {
                    let name = name.trim().to_string();
                    let password = password.trim().to_string();
    ...
    /nick NAME [PASSWORD]       Change your name.
